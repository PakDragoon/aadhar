...
App = {
    ...
    App.contracts.Contest.deployed().then(function (instance) {
      ...
      if (state == 2) {
        ...
        contestInstance.contestantsCount().then(function (contestantsCount) {
          for (var i = 1; i <= contestantsCount; i++) {
            contestInstance.contestants(i).then(async function (contestant) {
              var id = contestant[0];
              var name = contestant[1];
              var voteCount = contestant[2];
              var fetchedParty = contestant[3];
              ...
              var resultTemplate = "<tr><th>" + id + "</th><td>" + obj.name + "</td><td>" + fetchedAge + "</td><td>" + fetchedParty + "</td><td>" + fetchedQualification + "</td><td>" + voteCount + "</td></tr>";
  ...
  addCandidate: async function () {
    ...
    const response = await fetch('http://127.0.0.1:8000/addCandidate/?name=' + name + '&party=' + party + '&age=' + age + '&qualification=' + qualification);
